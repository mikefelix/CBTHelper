...
class Behavior3BFragment : BehaviorFragment(){

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? =
        FragmentBehaviorQuestion3Radios2QuestionsBinding.inflate(inflater, container, false).apply {
            buttons.setupButtons(enableNextWhenFilled = viewModel.healthValue)
            expression.displayDatum(viewModel.expression)

            question1.setText(R.string.behavior3Question)
            question2.setText(R.string.behavior3Question2)

            radioAnswer1.bindTo(viewModel.healthValue, R.string.behavior3Answer1, 1)
            radioAnswer2.bindTo(viewModel.healthValue, R.string.behavior3Answer2, 2)
            radioAnswer3.bindTo(viewModel.healthValue, R.string.behavior3Answer3, 3)
